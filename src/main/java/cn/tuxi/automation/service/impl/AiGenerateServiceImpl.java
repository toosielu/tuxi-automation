...
public class AiGenerateServiceImpl implements AiGenerateService {
    ...
    private final ViralContentTemplateLibrary templateLibrary;

    public AiGenerateServiceImpl(HistoryService historyService, ViralContentTemplateLibrary templateLibrary) {
    ...
    public GenerateResult generate(GenerateRequest request) {
        ...
        ViralContentTemplate selectedTemplate = templateLibrary.selectFor(goal, style);

        List<CoverPlan> covers = buildCovers(niche, productName, targetUser, style, selectedTemplate);
        List<XiaohongshuPost> posts = buildPosts(niche, productName, targetUser, goal, painPoint, sourceContext, selectedTemplate);
        ProductCopy productCopy = buildProductCopy(productName, targetUser, sellingPoints, selectedTemplate);
    ...
    private List<CoverPlan> buildCovers(String niche, String productName, String targetUser, String style, ViralContentTemplate template) {
        return List.of(
                new CoverPlan(
                        targetUserShort(targetUser) + "先别乱发",
                        "3 个低成本起号方向",
                        "干净白底 + 红色重点字 + 产品截图感",
                        "上方大标题，中间放资料包截图，下方放 3 个卖点标签；结构参考：" + template.coverFormula(),
                        "小红书 1:1 封面，白色背景，红色重点标题，展示" + productName + "、电脑界面、便签元素，干净高级，适合" + niche + "知识产品",
                        "标题直接击中新手痛点，信息少但明确，容易让正在找方向的人停下来。采用" + template.name() + "结构，点击理由更清晰。"
                ),
                new CoverPlan(
                        "这类资料更容易被收藏",
                        targetUserShort(targetUser) + "照着做",
                        "备忘录截图感 + 真实桌面 + 轻量标注",
                        "左侧放手机备忘录截图，右侧放产品交付清单，底部放领取提示；结构参考：" + template.bodyPattern(),
                ...
                new CoverPlan(
                        ...
                        "换个说法更像真人",
                        "对比图 + 手写批注 + 少量高亮",
                        "左右对比错误写法和改后写法，中间用箭头连接；结构参考：" + template.hookFormula(),
    ...
    private List<XiaohongshuPost> buildPosts(
            String niche,
            String productName,
            String targetUser,
            String goal,
        ...
        return List.of(
                new XiaohongshuPost(
                        ...
                        sourceContext + "如果你现在卡在“发什么、怎么卖”，先别急着把产品做得很大。按" + template.name() + "来写，第一步先指出一个具体错误动作，第二步给出替代做法，第三步用一条笔记测试用户会不会收藏、会不会问、会不会点进商品。只要这三个动作里有一个跑出来，再去补资料包和详情页，成功率会稳很多。",
                        "想要这套测试表，可以评论“测试”。",
                        List.of("#小红书运营", "#虚拟资料", "#副业起盘", "#内容变现"),
                        "这条用" + template.name() + "切入，先给问题再给动作，适合带来评论和私信。风险边界：" + template.riskNotice()
                ),
                new XiaohongshuPost(
                        "流量爆款贴",
                        "你以为缺流量，其实是承接断了",
                        "有些笔记数据不差，但就是没人买，问题通常不在标题。",
                        "我会先看最后一句：是不是太像广告，用户一看就想退出。更自然的做法是先给一个小清单，让对方确认自己是不是这个问题，再引导看产品或私信。尤其是" + goal + "，动作越轻，信任越容易起来。可以套用这个结构：错误做法 -> 改后说法 -> 为什么用户更愿意继续看。",
                        "需要低压 CTA 模板，留言“话术”。",
                        List.of("#小红书文案", "#私域转化", "#虚拟电商", "#引流文案"),
                        "反差标题有点击欲，正文把流量问题转成成交承接问题，适合店铺点击和私信咨询。"
                ),
                new XiaohongshuPost(
                        "信任成交贴",
                        productName + "适合谁？我会这样讲清楚",
                        "别只写资料很全，用户真正想看的是拿到后能干什么。",
                        "这份内容更适合" + targetUser + "。如果你现在的问题是" + painPoint + "，建议先按“选题、封面、正文、承接”四步跑一遍。资料只是工具，重点是让你当天就能发出一条像真人写的内容，而不是继续停在收藏教程。成交承接不要一上来推价格，先讲适合谁、怎么用、用完能得到什么可检查的小结果。",
    ...
    private ProductCopy buildProductCopy(String productName, String targetUser, String sellingPoints, ViralContentTemplate template) {
        return new ProductCopy(
                ...
                Arrays.stream(sellingPoints.split("[、,，\\n]"))
                        ...
                        .limit(6)
                        .toList(),
                "下单后交付文档、封面方案、文案模板和私信话术，可按当天项目直接套用。建议按" + template.name() + "先跑 3 条内容再复盘。",
